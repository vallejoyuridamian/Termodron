...
void setup(){
  ...
  sendATcommand(aux_str, "PB DONE", 30000);
  ...
  answer=0;
  //answer = sendATcommand("AT+NETOPEN=\"TCP\",2101", "OK", 20000);
  answer = sendATcommand("AT+CIPOPEN=0,\"TCP\",\"201.217.132.178\",2101,0", "OK", 20000);
  
  if (answer == 1){
    //Serial.println("Network opened");
    //sprintf(aux_str, "AT+TCPCONNECT=\"201.217.132.178\",2101", url, port);
    //answer = sendATcommand("AT+TCPCONNECT=\"201.217.132.178\",2101", "Connect ok", 20000);
...
void loop(){
  ...
  if (millis() > 50000 and fin){
    ...
    //Serial3.write(requestlock);
    //Serial.println(requestlock);
    //Serial3.write(0x1A);
...
void power_on(){
  ...
  if(not(listo)){
    Serial.println(F("Salio por timeout en la inicializacion"));
    digitalWrite(rstModulePin,HIGH);
    delay(100);
    digitalWrite(rstModulePin,LOW);
  }
...
void sendRTCM(byte bufferMSG[],int MSGLen){
   ...
   mavlink_msg_gps_inject_data_pack(system_id, component_id, &msg, target_system, target_component,MSGLen ,&bufferMSG[0]);
