...
void SPIClass::beginTransaction(uint8_t pin, SPISettings settings)
{
    ...
    Serial.print("Setting of port: ");
    Serial.print(pin);
    Serial.print("---");
    Serial.println(ch);
    Serial.print("config: ");
    Serial.println(settings.config);
    Serial.print("bit order: ");
    Serial.println(settings.border);
...
/*uint16_t SPIClass::transfer16(byte _pin, uint16_t _data, SPITransferMode _mode) {
	...
	uint32_t ch = BOARD_PIN_TO_SPI_CHANNEL(_pin);
	t.val = _data;
    t.lsb = (byte) 0x00FF & t.val;
    t.msb = (byte) 0x00FF & (t.val >> 8);
	...
	if (bitOrder[ch] == LSBFIRST) {
		...
		//t.msb = transfer(_pin, t.msb, SPI_CONTINUE);
		//t.lsb = transfer(_pin, t.lsb, _mode);
        t.val = transfer(_pin, t.msb, SPI_CONTINUE) << 8;
        t.val |= transfer(_pin, t.lsb, _mode);
	}
	return t.val;
}*/
