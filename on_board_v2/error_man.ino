...
void error_sync(void){
  ...
  if (myFile) {
    myFile.print(F("Error resync Lepton"));
    myFile.close();
  }
  #endif
  datasend_error(1);
...
void error_SDfile(char *file_error){
    ...
    if (myFile) {
      myFile.print(F("Error al generar archivo:"));
      myFile.println(file_error);
      myFile.println(F("Error solucionado, pero informacion perdida."));
      myFile.close();
    }else{
      datasend_error(2);
      reset_onboard();
...
void checkForErrors() {
    ...
    if (errorwire) {
      if (myFile) {
        myFile.print(F("Check Wire errors:"));
        sermon.print(F("Check Wire errors:"));
    ...
    if (errorlepton) {
      if (myFile) {
        myFile.print(F("Check Lepton errors:"));
